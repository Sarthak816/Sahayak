<template>
  <div class="flex w-full flex-col h-full bg-gray-50">
    ...
    <div class="border-b border-gray-200 bg-white/20 p-4 flex justify-end items-center">
      <button
        @click="resetChat"
        class="rounded-lg bg-red-500 px-4 py-2 text-sm text-white font-medium hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
      >
        Reset Chat
      </button>
    </div>
...
const finalUserAvatar = ref(props.userAvatar || loadUserAvatar())

// Load user avatar from localStorage or generate new one
...
// Watch messages array and save to localStorage on changes
watch(messages, () => {
  saveMessages()
